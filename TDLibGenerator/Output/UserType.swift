...
public extension TDEnum {
    enum UserType: TDEnumProtocol {
        ...
        /// user_id is available. It is not possible to perform any active actions
        ...
        /// supergroup chats
        ///  - canReadAllGroupMessages: True, if the bot can read all messages in
        /// basic group or supergroup chats and not just those addressed to the
        /// bot. In private and channel chats a bot can always read all messages
        ///  - isInline: True, if the bot supports inline queries
        ///  - inlineQueryPlaceholder: Placeholder for inline queries (displayed
        /// on the client input field)
        ///  - needLocation: True, if the location of the user should be sent with
        /// every inline query to this bot
        case bot(canJoinGroups: Bool, canReadAllGroupMessages: Bool, isInline: Bool, inlineQueryPlaceholder: String, needLocation: Bool)

        ///  No information on the user besides the user_id is available, yet this
        /// user has not been deleted. This object is extremely rare and must be
        /// handled like a deleted user. It is not possible to perform any actions
