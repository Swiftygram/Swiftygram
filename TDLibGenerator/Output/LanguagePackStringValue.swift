...
switch self {
case .deleted:
case .pluralized(let zeroValue, let oneValue, let twoValue, let fewValue, let manyValue, let otherValue):
case .ordinary(let value):
try container.encode("languagePackStringValueOrdinary", forKey: .init(string: "@type"))

try container.encode(value, forKey: .init(string: "value"))

try container.encode("languagePackStringValuePluralized", forKey: .init(string: "@type"))
...
try container.encode("languagePackStringValueDeleted", forKey: .init(string: "@type"))
