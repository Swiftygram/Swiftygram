...
//internal protocol _DictionaryStringDictionaryEncodableMarker { }
//#else
//fileprivate protocol _DictionaryStringDictionaryEncodableMarker { }
//#endif
//
//extension Dictionary : _DictionaryStringDictionaryEncodableMarker where Key == String, Value: Encodable { }
//
///// A marker protocol used to determine whether a value is a `String`-keyed `Dictionary`
...
//    open var keyEncodingStrategy: KeyEncodingStrategy = .convertToSnakeCase
//    
...
//        let encoder = __DictionaryEncoder(options: self.options)
//        
...
//            throw EncodingError.invalidValue(value,
//                                             EncodingError.Context(codingPath: [], debugDescription: "Top-level \(T.self) encoded as null Dictionary fragment."))
//        } else if topLevel is NSNumber {
//            throw EncodingError.invalidValue(value,
//                                             EncodingError.Context(codingPath: [], debugDescription: "Top-level \(T.self) encoded as number Dictionary fragment."))
//        } else if topLevel is NSString {
//            throw EncodingError.invalidValue(value,
//                                             EncodingError.Context(codingPath: [], debugDescription: "Top-level \(T.self) encoded as string Dictionary fragment."))
//        }
//        
...
//}
//
//// MARK: - __DictionaryEncoder
//
//// NOTE: older overlays called this class _DictionaryEncoder.
//// The two must coexist without a conflicting ObjC class name, so it
//// was renamed. The old name must not be used in the new runtime.
//fileprivate class __DictionaryEncoder : Encoder {
//    // MARK: Properties
//    
...
//    private let encoder: __DictionaryEncoder
//    
...
//    fileprivate init(referencing encoder: __DictionaryEncoder, codingPath: [CodingKey], wrapping container: NSMutableDictionary) {
...
//        return __DictionaryReferencingEncoder(referencing: self.encoder, key: _DictionaryKey.super, convertedKey: _converted(_DictionaryKey.super), wrapping: self.container)
//    }
//    
//    public mutating func superEncoder(forKey key: Key) -> Encoder {
//        return __DictionaryReferencingEncoder(referencing: self.encoder, key: key, convertedKey: _converted(key), wrapping: self.container)
//    }
//}
//
...
//    
//    /// Initializes `self` with the given references.
//    fileprivate init(referencing encoder: __DictionaryEncoder, codingPath: [CodingKey], wrapping container: NSMutableArray) {
//        self.encoder = encoder
//        self.codingPath = codingPath
//        self.container = container
//    }
...
//    
//    public mutating func superEncoder() -> Encoder {
//        return __DictionaryReferencingEncoder(referencing: self.encoder, at: self.container.count, wrapping: self.container)
//    }
//}
//
//extension __DictionaryEncoder : SingleValueEncodingContainer {
...
//            
//            if let baseObjectType = type as? TGBaseObject.Type {
//                var container = self.storage.popContainer(); defer { self.storage.push(container: container) }
//                
//                if let dict = container as? NSMutableDictionary {
//                    dict["@type"] = baseObjectType.type
//                }
////                var container = self.container(keyedBy: TGTypeKeys.self)
////
////                try container.encode(baseObjectType.type, forKey: .type)
//            }
//        } catch {
//            // If the value pushed a container before throwing, pop it back off to restore state.
//            if self.storage.count > depth {
...
//}
//
//// MARK: - __DictionaryReferencingEncoder
//
///// __DictionaryReferencingEncoder is a special subclass of __DictionaryEncoder which has its own storage, but references the contents of a different encoder.
///// It's used in superEncoder(), which returns a new encoder for encoding a superclass -- the lifetime of the encoder should not escape the scope it's created in, but it doesn't necessarily know when it's done being used (to write to the original container).
//// NOTE: older overlays called this class _DictionaryReferencingEncoder.
//// The two must coexist without a conflicting ObjC class name, so it
//// was renamed. The old name must not be used in the new runtime.
//fileprivate class __DictionaryReferencingEncoder : __DictionaryEncoder {
...
//    fileprivate let encoder: __DictionaryEncoder
//    
...
//    fileprivate init(referencing encoder: __DictionaryEncoder, at index: Int, wrapping array: NSMutableArray) {
//        self.encoder = encoder
...
//    fileprivate init(referencing encoder: __DictionaryEncoder,
...
//    open var keyDecodingStrategy: KeyDecodingStrategy = .convertFromSnakeCase
//    
...
//        let decoder = __DictionaryDecoder(referencing: topLevel, options: self.options)
...
//    private let decoder: __DictionaryDecoder
//    
...
//    fileprivate init(referencing decoder: __DictionaryDecoder, wrapping container: [String : Any]) {
...
//        return __DictionaryDecoder(referencing: value, at: self.decoder.codingPath, options: self.decoder.options)
//    }
//    
...
//    
//    /// Initializes `self` by referencing the given decoder and container.
//    fileprivate init(referencing decoder: __DictionaryDecoder, wrapping container: [Any]) {
//        self.decoder = decoder
//        self.container = container
//        self.codingPath = decoder.codingPath
//        self.currentIndex = 0
...
//            
//            if let baseObjectType = type as? TGBaseObject.Type {
//                if let dict = value as? [String: Any], let responseType = dict["@type"] as? String, baseObjectType.type != responseType {
//                    throw DecodingError._typeMismatch(at: self.codingPath, expectation: type, reality: value)
//                }
//            }
//            
...
//        return DecodingError.typeMismatch(expectation, DecodingError.Context(codingPath: path, debugDescription: ""))
//    }
//    
//}
